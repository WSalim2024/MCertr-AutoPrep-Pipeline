...
- **Data Quality:** Validation of logic, duplicates, and consistency.

## ✨ Features [v1.2.0]
- **🛡️ Quality Control:**
  - **Duplicate Removal:** Automatically detects and drops repeated rows.
  - **Logic Validation:** Identifies impossible values (e.g., negative ages) and flags them.
  - **Consistency Checks:** Cross-validates related columns (e.g., `Total == Part1 + Part2`).
  - **Text Standardization:** Fixes inconsistencies like "paris" vs "Paris".
- **📉 Visual Diagnostics:** Interactive Box Plots to spot outliers visually.
- **📊 Interactive Dashboard:** Streamlit UI for real-time testing.
- **⚡ Automated Ingestion:** Loads CSV datasets seamlessly.
- **🧹 Smart Cleaning:** Handles missing values and outliers (Z-score).
- **📏 Normalization:** Uses **StandardScaler** (Z-score normalization).
